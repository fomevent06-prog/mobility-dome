...
exports.EyGuideDataService = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const pdf_parse_1 = require("pdf-parse");
const GUIDE_PAGE_URL = "https://www.ey.com/en_gl/technical/tax-guides/worldwide-personal-tax-and-immigration-guide";
function normalizeText(text) {
    return text.replace(/\r/g, "").replace(/\n{3,}/g, "\n\n").trim();
}
function resolvePdfUrl(landingHtml) {
...
function chunkText(text, source, chunkSize = 1800, overlap = 220) {
    const docs = [];
    let i = 0;
    let idx = 0;
    while (i < text.length) {
...
function extractCountryPageStarts(text) {
    const starts = [];
    ...
    while ((m = re.exec(toc)) !== null) {
        ...
        if (!country || Number.isNaN(page))
            continue;
        if (/^(About|EY Global Tax contacts|Contacts for other jurisdictions|Currencies|EY Global People|Preface|Contents)/i.test(country))
            continue;
        starts.push({ country, page });
    }
    const dedup = new Map();
    for (const s of starts) {
        if (!dedup.has(s.country))
            dedup.set(s.country, s.page);
    }
    return [...dedup.entries()].map(([country, page]) => ({ country, page })).sort((a, b) => a.page - b.page);
}
function extractSectionByPage(text, startPage, endPage) {
...
function buildDocsFromText(text) {
...
function buildCountrySentimentDocs(docs, countries) {
    const sentimentMap = {};
    const sentimentDocs = [];
...
class EyGuideDataService {
    constructor(dataDir) {
        this.dataDir = dataDir;
        this.docs = [];
        this.countriesList = [];
        this.countrySentiment = {};
        this.cachePath = path_1.default.join(this.dataDir, "_ey_worldwide_guide_cache.json");
    }
    async load() {
        try {
            const landing = await fetch(GUIDE_PAGE_URL);
            if (!landing.ok)
                throw new Error(`Landing page fetch failed: ${landing.status}`);
            const html = await landing.text();
            const pdfUrl = resolvePdfUrl(html);
            if (!pdfUrl)
                throw new Error("Could not resolve EY guide PDF URL from landing page.");
            const pdfResponse = await fetch(pdfUrl);
            if (!pdfResponse.ok)
                throw new Error(`Guide PDF fetch failed: ${pdfResponse.status}`);
            const pdfBytes = Buffer.from(await pdfResponse.arrayBuffer());
            const parser = new pdf_parse_1.PDFParse({ data: pdfBytes });
            ...
            if (!docs.length)
                throw new Error("No text chunks were extracted from EY guide PDF.");
            ...
            fs_1.default.writeFileSync(this.cachePath, JSON.stringify(cache));
        }
        catch (error) {
            if (!fs_1.default.existsSync(this.cachePath)) {
                throw error;
            }
            const cached = JSON.parse(fs_1.default.readFileSync(this.cachePath, "utf-8"));
    ...
    insights(country) {
        ...
        const simple = (word) => scoped.filter((d) => d.text.toLowerCase().includes(word)).length;
        const confidence = assigneeCount >= 25 ? "high" : assigneeCount >= 12 ? "medium" : assigneeCount >= 5 ? "low" : "very_low";
        return {
...
exports.EyGuideDataService = EyGuideDataService;
